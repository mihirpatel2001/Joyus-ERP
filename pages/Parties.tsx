            ...
            <table className="w-full text-sm text-left">
              <thead className="bg-canvas text-content-sub font-medium border-b border-divider sticky top-0 z-10 shadow-sm">
                <tr>
                  ...
                  <th className="px-6 py-4 whitespace-nowrap bg-canvas">Contact</th>
                  <th className="px-6 py-4 whitespace-nowrap text-right bg-canvas">Receivables</th>
                  <th className="px-6 py-4 whitespace-nowrap text-right bg-canvas">Payables</th>
                  <th className="px-6 py-4 whitespace-nowrap text-center bg-canvas">Status</th>
                  <th className="px-6 py-4 text-right whitespace-nowrap bg-canvas">Action</th>
                </tr>
              </thead>
          ...
          <div className="md:hidden">
            {currentItems.length > 0 ? (
              <div className="divide-y divide-divider">
                {currentItems.map((party) => (
                  <div key={party.id} className="p-4 space-y-3 bg-surface hover:bg-surface-highlight transition-colors">
                    <div className="flex justify-between items-start">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h3 className="font-semibold text-content-strong">{party.name}</h3>
                    ...
                    <div className="text-sm text-content-sub space-y-1 pl-1">
                      <div className="flex items-center gap-2">
                    ...
                    <div className="grid grid-cols-2 gap-3 pt-2">
                      <div className="bg-canvas p-2 rounded-lg">
                        <p className="text-xs text-content-sub mb-0.5">Receivables</p>
                        <p className={`font-medium ${party.receivables > 0 ? 'text-success' : 'text-content-normal'}`}>
                          {party.receivables > 0 ? `₹ ${party.receivables.toLocaleString()}` : '-'}
                        </p>
                      </div>
                      <div className="bg-canvas p-2 rounded-lg text-right">
                        <p className="text-xs text-content-sub mb-0.5">Payables</p>
                        <p className={`font-medium ${party.payables > 0 ? 'text-danger' : 'text-content-normal'}`}>
                          {party.payables > 0 ? `₹ ${party.payables.toLocaleString()}` : '-'}
                        </p>
                      </div>
                    </div>
              ...
              <div className="p-8 text-center text-content-sub">
                No parties found matching your search.
              </div>
            )}
          </div>
